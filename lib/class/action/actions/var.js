...
module.exports = class VarPluginAction extends PluginAction {
  ...
  replace(plugin, files) {
    plugin.Logger('mvr: VarPluginAction...');
    ...
    Object.keys(files).forEach(function(path) {
      ...
      if (PluginAction.fileFilter.test(path)) {
        ...
        if (PluginAction.keyRegex.test(file.contents.toString())) {
          plugin.Logger('mvr (var): updating ' + path + ' file');
          var err = '';
          //Do the replacement
          file.contents = file.contents
            ...
            .replace(PluginAction.keyRegex, (match, param) => {
              ...
              err = 'mvr (var): "' + param + '" variable not found.';
              plugin.Logger(err);
    ...
    plugin.Logger('mvr: VarPluginAction complete.');
    return files;
  }
}
