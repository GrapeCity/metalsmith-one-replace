...
function varRepPlugin(config) {
    ...
    config.consoleOutput = typeof config.consoleOutput === 'boolean' ? config.consoleOutput : false;
    config.enabled = typeof config.enabled === 'boolean' ? config.enabled : false;
    if (!config.enabled) {
        if (config.consoleOutput) console.log('metalsmith-var-replace: disabled at config level');
        return;
    }
    config.actions = typeof config.actions === 'undefined' ? [] : config.actions;
    if (config.actions.length < 1) {
        if (config.consoleOutput) console.log('metalsmith-var-replace: no actions to perform');
        return;
    }
...
function varRepProcess(config, files, metalsmith) {
    if (config.consoleOutput) console.log('metalsmith-var-replace (mvr): start... [' + Date.now() + ']');

    config.actions.sort(actionCompare).forEach(action => {
        if (typeof action === 'object') {
            if ((typeof action.type === 'string') && (actionTypes.includes(action.type))) {
                if (config.consoleOutput) console.log('mvr: action type - ' + action.type);
                switch (action.type) {
                    case 'var':
                        ...
                        varRepAction(config.consoleOutput, action, files);
                        break;
                    case 'file':
                        action.filePattern = typeof action.filePattern === 'string' ? action.filePattern : defaults.filePattern;
                        action.ignoreMissing = typeof action.ignoreMissing === 'boolean' ? action.ignoreMissing : false;
                    ...
                    case 'replace':
                        ...
                        if (validAction){ varRegAction(config.consoleOutput, action, files); }
                        else if (config.consoleOutput) console.log('mvr: missing/invalid remove type action settings [json: ' + JSON.stringify(action) + ']');
                        break;
                }
            }
            else if (config.consoleOutput) console.log('mvr: missing/invalid action type [json: ' + JSON.stringify(action) + ']');
        }
    });
    if (config.consoleOutput) console.log('metalsmith-var-replace (mvr): end. [' + Date.now() + ']');
}
